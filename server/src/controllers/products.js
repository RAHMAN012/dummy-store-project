...
export const createProducts = async (req, res, next) => {
  const { title, description, price, images, category, brand } = req.body;
  try {
...
export const getProductsByCategory = async (req, res, next) => {
  ...
  try {
    ...
    if (!products) {
      return next(
        createHttpError(404, `Product not found matching ${categoryName}`)
      );
    }
...
export const updateProducts = async (req, res, next) => {
  const { title, description, price, images, category, inStock, brand } =
    req.body;
  ...
  try {
    ...
    Object.keys(updatedFields).forEach((key) => {
      if (
        ...
        Array.isArray(updatedFields[key].length === 0)
      ) {
...
export const deleteProduct = async (req, res, next) => {
  ...
  try {
    if (!productId) {
      createHttpError(400, "Product id is missing");
    }
    const product = await Product.findById(productId);
    if (!product) {
      return next(createHttpError(404, "Product not found"));
